...
function calculateDamagePercent() {
  ...
  villains.forEach((villain) => {
    damagePercent = (villain.health / villain.maxHealth) * 100;
  });
...
function drawVillain() {
  for (let i = 0; i < villains.length; i++) {
    ...
    villainElem.querySelector(
      ".villain-level"
    ).innerHTML = `LVL ${villain.level}`;
    let healthElem = villainElem.querySelector(".progress");
    healthElem.innerHTML = `<div class="progress-bar progress-bar-striped bg-danger progress-bar-animated healthbar" role="progressbar" aria-valuenow="${calculateDamagePercent().toString()}" aria-valuemin="0" aria-valuemax="${
      villain.maxHealth
    }" style="width: ${calculateDamagePercent()}%">VADER</div>`;
  }
}
